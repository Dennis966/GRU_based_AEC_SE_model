...
from data_generator_SE import wav_Dataset
from build_model import *
from utils_SE import model_detail_string, train, test, cal_time
...
parser.add_argument('--model', type=str, default='SE_BSRU', help='options: SE_BSRU')
...
if __name__ == '__main__':
    ...
    if model_name == 'SE_BSRU':
        model = SE_BSRU().to(device)
        input_size = [train_batch_size, 257, 150] # 150 is just an example for frame length of spectrogram
    elif model_name == 'SE_BSRU4':
        model = SE_BSRU4().to(device)
        input_size = [train_batch_size, 257, 150] # 150 is just an example for frame length of spectrogram
    elif model_name == 'SE_BSRU8':
        model = SE_BSRU8().to(device)
        input_size = [train_batch_size, 257, 150] # 150 is just an example for frame length of spectrogram
    # elif model_name == 'NEW_MODEL':
    #     model = NEW_MODEL().to(device)
    #     input_size = [train_batch_size, 257, 150] # 150 is just an example for frame length of spectrogram
    elif model_name == 'First_SE_model_4':
        model = First_SE_model_4().to(device)
        input_size = [train_batch_size, 257, 150]

    else:
        raise NameError('custom models (with the parameters of input_size) should be written in main_SE.py,\n'
